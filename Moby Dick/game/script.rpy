...
# The game starts here. The label "start" is a preset, which we preserve for clarity. However, we immediately pass through to etymology, the actual first section of the game.
label start:
# typesetting is done with comparisons to the first american edition: https://archive.org/details/mobydickorwhale01melv/
# there's a plain-text copy to work from (note to self: should transform this from html to renpy later)
init python:
...
{font=fonts/times.ttf}וח{/font},                        {space=300} Hebrew.\n
...
label extracts:
  ...
  chapter "EXTRACTS.\n{font=fonts/MonAmourFraktur-Broken.ttf}({bs}s{/bs}upplied by a {bs}s{/bs}ub={bs}s{/bs}ub=Librarian.){/font}"
label c1:
